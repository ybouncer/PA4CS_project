...
private class ExampleZeroAnalysisFlowFunction extends
  ...
  override def visitExampleArrayAccess(
    node: ExampleArrayAccess,
    abstractEnvironment: ExampleAbstractEnvironment[String, ExampleZeroAnalysisAbstractValue]
  ): Try[ExampleZeroAnalysisAbstractValue] = {
    Try {
      abstractEnvironment.abstractValues(node.arrayName) match {
        case ExampleZeroAnalysisAbstractValue.Z => ExampleZeroAnalysisAbstractValue.Z
        case ExampleZeroAnalysisAbstractValue.NZ => ExampleZeroAnalysisAbstractValue.NZ
        case _ => ExampleZeroAnalysisAbstractValue.U
      }
    }
  }

  override def visitExampleArrayAssign(
    node: ExampleArrayAssign,
    abstractEnvironment: ExampleAbstractEnvironment[String, ExampleZeroAnalysisAbstractValue]
  ): Try[ExampleZeroAnalysisAbstractValue] = {
    for {
      value <- node.value.accept(this, abstractEnvironment)
      _ = abstractEnvironment.set(node.arrayName, value) // Met à jour l'environnement
    } yield value // Retourne la valeur assignée
  }
...
private class ConditionUpdateFunction() extends ExampleExpressionVisitor[Try[Option[Map[String, ExampleZeroAnalysisAbstractValue]]], ExampleAbstractEnvironment[String, ExampleZeroAnalysisAbstractValue]] {
  ...
  override def visitExampleArrayAccess(node: ExampleArrayAccess, abstractEnvironment: ExampleAbstractEnvironment[String, ExampleZeroAnalysisAbstractValue]): Try[Option[Map[String, ExampleZeroAnalysisAbstractValue]]] = {
    Success(Some(Map.empty)) // Exemple simplifié, ajustez selon vos besoins
  }

  override def visitExampleArrayAssign(
    node: ExampleArrayAssign,
    abstractEnvironment: ExampleAbstractEnvironment[String, ExampleZeroAnalysisAbstractValue]
  ): Try[Option[Map[String, ExampleZeroAnalysisAbstractValue]]] = {
    for {
      value <- node.value.accept(this, abstractEnvironment)
    } yield value // Retourne la mise à jour dans une Option
  }
